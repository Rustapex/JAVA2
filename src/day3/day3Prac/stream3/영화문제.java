...
public class 영화문제 {
	...
	public static void main(String[] args) {
		...
		movies.stream().forEach(item -> System.out.println(item.getDirector()));
		...
		List<String> list1 = movies.stream().map(item -> item.getTitle()).toList();
		System.out.println(list1);

		List<String> list2 = movies.stream().map(item -> item.getTitle()).collect(Collectors.toList());
		...
		// 6.정렬( Comparator )
		movies.stream()
		.sorted(Comparator.<Movie, Integer>comparing(item -> item.getYear()))
		.forEach(item -> System.out.println(item));
